  ...
  <div class="min-h-screen bg-[#f5f0eb] flex items-center justify-center p-5">
    <div class="w-full max-w-[420px] bg-white border-2 md:border-4 border-[#3d3d3d] shadow-[4px_4px_0px_0px_rgba(61,61,61,0.10)] md:shadow-[8px_8px_0px_0px_rgba(61,61,61,0.10)] p-8 md:p-10">
      ...
      <div class="text-center mb-8">
        ...
        <p class="font-mono text-sm text-gray-500">注册一个新的知识库平台账号</p>
      </div>
      ...
      <form class="flex flex-col gap-5" @submit.prevent="handleRegister">
        ...
        <div>
          <AppInput
            ...
            placeholder="请输入邮箱"
          />
          <p v-if="errors.email" class="font-mono text-xs text-[#5a7a6b] mt-1">{{ errors.email }}</p>
        </div>
        ...
        <div>
          <AppInput
            ...
            placeholder="请再次输入密码"
          />
          <p v-if="errors.confirmPassword" class="font-mono text-xs text-[#5a7a6b] mt-1">{{ errors.confirmPassword }}</p>
        </div>
      ...
      <div class="text-center mt-5 font-mono text-xs text-gray-500">
        <span>已有账号？</span>
        <router-link to="/login" class="text-[#3d3d3d] underline hover:text-[#5a7a6b] transition-colors ml-1">
          立即登录
        </router-link>
      </div>
    </div>
